...
**一键起服务**：
```bash
./run.sh          # 拉起 postgres + 后端(:8000) + 前端(:3000),Ctrl+C 一并停止
```
首次用先去 http://localhost:3000/settings 接入模型（Claude 订阅登录 或 API Key）。
（手动起：`docker compose up -d` → `cd server && go run .` → `cd frontend && pnpm dev`）
