...
## 当前状态
**阶段 1（生产地基）已完成**（2026-06）。
- 多租户后端：Postgres + RLS 行级隔离、对话落库、BYO-key 加密存储、dev 鉴权 shim（预留 Clerk）
- 前端：Next.js 16 + Clerk（dev fallback）、频道/流式对话/历史持久化、组织设置
- 测试：pytest（含 RLS 隔离用例）+ GitHub Actions CI（后端测试 + 前端构建）

本地起步见 [DEPLOY.md](DEPLOY.md)。下一步：阶段 2（Agent 工具调用 + LangGraph）。
